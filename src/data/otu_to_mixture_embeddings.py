...
def load(otu_tables_path, id_to_embedding_path, model_path, sample_data_to_sample_id_path):
...
def main(otu_tables_path, id_to_embedding_path, model_path, out_path, sample_data_to_sample_id_path):
    ...
    results = load(otu_tables_path, id_to_embedding_path, model_path, sample_data_to_sample_id_path)
    otu_tables, id_to_embedding, distance_str, radius, sample_data_to_sample_id = results
    embeddings_normed = get_normalized_embeddings(otu_tables, id_to_embedding, distance_str, radius)
    
    print('Computing mixture embeddings seperately for each of the 10 sample body sites...')
    sample_id_to_mixture_embedding = get_mixture_embeddings(otu_tables, embeddings_normed, sample_data_to_sample_id)
    save(sample_id_to_mixture_embedding, out_path)
    return sample_id_to_mixture_embedding
    
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--otu_tables_path', type=str, default='data/interim/moms_pi/otu_tables_normed_and_cleaned.pickle')
    ...
    main(args.otu_tables_path, args.id_to_embedding_path, args.model_path, args.out_path, args.sample_data_to_sample_id_path)